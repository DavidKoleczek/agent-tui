...
function ApprovalCard({ task, onDecide, onExpandTask }: ApprovalCardProps) {
    ...
    return (
        <box flexDirection="column" flexShrink={0}>
            <box flexDirection="row" columnGap={1}>
                <ApprovalButton label={formatTaskName(task.name)} bold truncate onPress={() => onExpandTask(task.id)} />
...
export function PendingApprovals({ tasks, onPermissionChange, onExpandTask }: PendingApprovalsProps) {
    return (
        <box flexDirection="column" flexShrink={0} marginTop={1}>
            ...
            {tasks.map((task, index) => (
                <box key={task.id} flexDirection="column" flexShrink={0} marginTop={index === 0 ? 0 : 1}>
                    ...
                    <ApprovalCard task={task} onDecide={onPermissionChange} onExpandTask={onExpandTask} />
                </box>
            ))}
        </box>
    )
