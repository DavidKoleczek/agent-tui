...
const HIDDEN_STATUS_IDS = ["agent_running", "agent_turn_ended"] as const satisfies readonly StatusId[]
...
interface StatusLineProps {
    ...
    ready: boolean
    // The latest server lifecycle status. agent_running and agent_turn_ended render without a label.
...
export function StatusLine({ ready, status, onBack }: StatusLineProps) {
    ...
    const dots = useDotBounce(label !== "" && (!ready || !isNonAnimatedStatus(status)))
