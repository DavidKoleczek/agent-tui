    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="approval-dialog"):
            ...
            with Vertical(id="feedback-section"):
                yield Input(placeholder="Provide feedback (denies all).", id="feedback-input")
...
class ToolCallItem(Horizontal):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Button("\u2713", variant="success", id=f"accept-{self.call_id}", classes="tool-btn")
        yield Button("\u2717", variant="error", id=f"reject-{self.call_id}", classes="tool-btn")
